...
var showAlert = function showAlert(type, mes) {
  hiddenAlert();
...
var displayMap = function displayMap(locArr) {
  mapboxgl.accessToken = 'pk.eyJ1IjoiZGRldmsiLCJhIjoiY2twbnJ4MzYyMjMxbDJ2bzhqdDNxeGpuOSJ9.H5CYIoW16i7A4NI4ht76ng';
  ...
  var bounds = new mapboxgl.LngLatBounds(); //MAKER tich hop san

  locArr.forEach(function (loc) {
    new mapboxgl.Marker().setLngLat(loc.coordinates).setPopup(new mapboxgl.Popup({
      offset: 30
    }).setHTML("<h3>Day ".concat(loc.day, ": ").concat(loc.description, "</h3>"))).addTo(map);
      ...
      while (1) {
        switch (_context.prev = _context.next) {
          ...
          case 9:
            _context.prev = 9;
            _context.t0 = _context["catch"](0);
            (0, _alert.showAlert)('error', "Please check ".concat(type, " again \uD83D\uDCA5\uD83D\uDCA5\uD83D\uDCA5"));
...
var formUserDataUpdate = document.querySelector('.form-user-data');
...
if (mapEle) {
  var locArr = JSON.parse(mapEle.dataset.locations);
  (0, _map.displayMap)(locArr);
}

if (logoutEle) {
  logoutEle.addEventListener('click', _login.logout);
}

if (formUserDataUpdate) {
  formUserDataUpdate.addEventListener('submit', /*#__PURE__*/function () {
